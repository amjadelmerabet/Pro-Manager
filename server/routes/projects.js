  ...
  if (method === "GET") {
          ...
          if (Object.keys(updates).length === 0) {
            ...
            if (!invalidUpdate) {
              ...
              if (updatedProject.status === "success") {
                res.writeHead(200, { "Content-Type": "application/json" });
                res.end(JSON.stringify({ message: "Project updated successfully" }));
              } else {
                if (updatedProject.message === "404 Project not found") {
                  res.writeHead(404, { "Content-Type": "application/json" });
                  res.end(JSON.stringify({ message: "There is no project with the id " + projectId }));
                } else {
                  res.writeHead(400, { "Content-Type": "application/json" });
                  res.end(JSON.stringify({ message: "There was an error updating the project with the id " + projectId }))
                }
              }
            }
          }
    ...
    if (url.match(/^\/projects\/delete\/.+/)) {
      ...
      if (deletedProject.status === "success") {
        ...
        res.writeHead(400, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ message: "There was an error deleting the project with the id " + projectId }));
      }
    } else {
      res.writeHead(404, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ message: "The endpoint that you are trying to reach doesn't exist" }));
